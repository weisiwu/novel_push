...
const platform_upload_video = async (platform, videoInfo = {}) => {
  ...
  const data = await mainPage.evaluate(
    async (params) => {
       ...
       * 视频投稿送审批
       */
      const bilibili_video_draft_auditing = () => {
        const draft_params = {
          cover:
      ...
      fileInputElement.addEventListener('change', async (event) => {
        ...
        if (file) {
          ...
          console.log('wswTest: 建立上传视频任务', video_task_info)
          // S2: 然后会对file_meta.txt建立上传任务，固定大小2000
          // 同时这个任务参数很固定
          // const file_mate_task_info = await bilibili_create_update_task({
          //   name: 'file_meta.txt',
          //   size: 2000,
          //   r: 'upos',
          //   profile: 'fxmeta%2Fbup',
          //   ssl: 0,
          //   build: '2140000',
          //   version: '2.14.0.0',
          //   webVersion: '2.14.0'
          // })
          // const { upos_uri: meta_upos_uri } = file_mate_task_info || {}
          // console.log('wswTest: 建立上传meta_file任务', file_mate_task_info)
          ...
          // console.log('wswTest: ', '不上传视频文件原始信息')
