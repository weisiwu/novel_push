...
app.whenReady().then(() => {
  ...
  ipcMain.on('cut-video', async (event, filePath) => {
    // TODO:(wsw) 联调反推tag
    if (mainWindow) {
      console.log('wswTest: ', '开始主流程')
      const imgTest = Buffer.from(readFileSync(filePath)).toString('base64')
      // fs.writeFileSync('testetsetst', Buffer.from(readFileSync(filePath)).toString('base64'))
      axios
        .post(
          'http://novel-push-1.1229125983044594.cn-shanghai.pai-eas.aliyuncs.com/sdapi/v1/interrogate?Token=NWU1ZDU0ZWIxMDNiOTdiY2Y1YzkzZGE4YmNkNWMzNjBhY2I5MDg2Mg==',
          {
            image: imgTest,
            image_url: imgTest,
            model: 'clip'
